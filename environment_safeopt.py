...
def make_dir(env_name, start_point, num_exp, max_iter, hyperparam):
    ...
    path = './benchmark_map/' + env_name + '/SafeOPT_' + str(max_iter) + '/l_' + str(hyperparam[0]) + '_b_' + str(hyperparam[1])
    ...
    if not (os.path.isdir(os.path.expanduser('./benchmark_map/' + env_name + '/SafeOPT_' + str(max_iter)))):
        os.makedirs(os.path.join(os.path.expanduser('./benchmark_map/' + env_name + '/SafeOPT_' + str(max_iter))))
    ...
    if not(os.path.exists(path + '/final_result_SafeOPT.xlsx')):
        final_result = [['start pint', 'num exp', 'time', 'esti best', 'safe rate', 'min regret']]
        df1 = pd.DataFrame.from_records(final_result)
        df1.to_excel(path + '/final_result_SafeOPT.xlsx', index=False)
    if not (os.path.exists(path + '/final_regret_SafeOPT.xlsx')):
        df2 = pd.DataFrame.from_records([])
        df2.to_excel(path + '/final_regret_SafeOPT.xlsx', index=False)
    if not (os.path.exists(path + '/final_cumregret_SafeOPT.xlsx')):
        df3 = pd.DataFrame.from_records([])
        df3.to_excel(path + '/final_cumregret_SafeOPT.xlsx', index=False)

    sys.stdout = open(path + '/' + str(start_point) + '/' + str(num_exp) + '/SafeOPT_log.txt', 'w')

def save_result(path, final_result, regret_list):

    df1 = pd.read_excel(path + '/final_result_SafeOPT.xlsx')
    df1 = df1.append(final_result)
    writer1 = pd.ExcelWriter(path + '/final_result_SafeOPT.xlsx', engine='openpyxl')
    df1.to_excel(writer1, index=False)
    writer1.save()

    df2 = pd.read_excel(path + '/final_regret_SafeOPT.xlsx')
    df2 = df2.append([np.array(regret_list)[:,0].T.tolist()])
    writer2 = pd.ExcelWriter(path + '/final_regret_SafeOPT.xlsx', engine='openpyxl')
    df2.to_excel(writer2, index=False)
    writer2.save()

    df3 = pd.read_excel(path + '/final_cumregret_SafeOPT.xlsx')
    df3 = df3.append([np.array(regret_list)[:,1].T.tolist()])
    writer3 = pd.ExcelWriter(path + '/final_cumregret_SafeOPT.xlsx', engine='openpyxl')
...
class Testbenchmark(object):
    def __init__(self, name, max_iter, num_exp, hyperparam, threshold, start_point):
        ...
        self.path = './benchmark_map/' + self.name + '/SafeOPT_' + str(max_iter) + '/l_' + str(self.hyperparam[0]) + '_b_' + str(self.hyperparam[1])
        ...
        self.opt = safeopt.SafeOptSwarm(gp=gp, fmin=self.fmin, bounds=self.bound, beta=hyperparam[1],
...
if __name__=='__main__':
    print("SafeOPT Load Options")
    args = options()

    "--max_iter, --num_exp, --length_scale, --beta, --env_name, --threshold, --start_point"
    ...
    env_setting = Testbenchmark(env_name, max_iter, num_exp, hyperparam, threshold, start_point)

...
def options():
    parser = argparse.ArgumentParser(description='GP-UCB experiments')
...
def make_dir(env_name, start_point, num_exp, max_iter, n_sample):
    ...
    path = './benchmark_map/' + env_name + '/GP-UCB_' + str(max_iter) + '_N_'+ str(n_sample)
    ...
    if not(os.path.exists(path + '/final_result_GP-UCB.xlsx')):
        final_result = [['start pint', 'num exp', 'time', 'esti best', 'safe rate', 'min regret']]
        df1 = pd.DataFrame.from_records(final_result)
        df1.to_excel(path + '/final_result_GP-UCB.xlsx', index=False)
    if not (os.path.exists(path + '/final_regret_GP-UCB.xlsx')):
        df2 = pd.DataFrame.from_records([])
        df2.to_excel(path + '/final_regret_GP-UCB.xlsx', index=False)
    if not (os.path.exists(path + '/final_cumregret_GP-UCB.xlsx')):
        df3 = pd.DataFrame.from_records([])
        df3.to_excel(path + '/final_cumregret_GP-UCB.xlsx', index=False)

    sys.stdout = open(path + '/' + str(start_point) + '/' + str(num_exp) + '/GP-UCB_log.txt', 'w')

def save_result(path, final_result, regret_list, hyp, mb):

    df1 = pd.read_excel(path + '/final_result_GP-UCB.xlsx')
    df1 = df1.append(final_result)
    writer1 = pd.ExcelWriter(path + '/final_result_GP-UCB.xlsx', engine='openpyxl')
    df1.to_excel(writer1, index=False)
    writer1.save()

    df2 = pd.read_excel(path + '/final_regret_GP-UCB.xlsx')
    df2 = df2.append([np.array(regret_list)[:,0].T.tolist()])
    writer2 = pd.ExcelWriter(path + '/final_regret_GP-UCB.xlsx', engine='openpyxl')
    df2.to_excel(writer2, index=False)
    writer2.save()

    df3 = pd.read_excel(path + '/final_cumregret_GP-UCB.xlsx')
    df3 = df3.append([np.array(regret_list)[:,1].T.tolist()])
    writer3 = pd.ExcelWriter(path + '/final_cumregret_GP-UCB.xlsx', engine='openpyxl')
...
class Testbenchmark(object):
    def __init__(self, name, max_iter, num_exp, threshold, start_point, n_sample):
        ...
        self.path = './benchmark_map/' + env_name + '/GP-UCB_' + str(max_iter) + '_N_' + str(self.n_sample)
        ...
        self.opt = gpucb(gp=gp, bounds=self.bound, threshold=self.threshold, max_iters=self.max_iters, name=self.name, n_samples=self.n_sample)
    ...
    def plot_initial_setting(self):
        ...
        if self.env.dim == 2:
            ...
            plt.savefig(self.path + '/' + str(start_point) + '/' + str(num_exp) + '/contour_safe_fig_00.png')
            plt.close()
        else:
    ...
    def plot(self, iter):
        ...
        if self.env.dim == 2:
            ...
            plt.savefig(self.path + '/' + str(start_point) + '/' + str(num_exp) + '/contour_safe_fig_%02d.png' % iter)
            plt.close()
    ...
    def plot_for_high_dim(self, regret_list):
        ...
        plt.title('regret')
        plt.savefig(self.path + '/' + str(start_point) + '/' + str(num_exp) + '/regret.png')
        plt.close()

        plt.plot(x, a_regret_list[:, 1])
        plt.title('cum_regret')
        plt.savefig(self.path + '/' + str(start_point) + '/' + str(num_exp) + '/cum_regret.png')
        plt.close()
        ...
        print(
            "Estimated target best points : {}, Estimated target best result : {}".format(max_point, self.esti_best))
    ...
    def best_plot(self):
        ...
        if self.env.dim == 2:
            ...
            plt.savefig(self.path + '/' + str(start_point) + '/' + str(num_exp) + '/contour_max_fig.png')
            plt.close()
...
if __name__ == '__main__':
    ...
    final_result = [[start_point, num_exp, float((t1 - t)), float(env_setting.esti_best), float(env_setting.safe_rate), float(min(np.array(env_setting.regret_list)[:,0]))]]
    save_result(env_setting.path, final_result, env_setting.regret_list, env_setting.opt.hyper_list, env_setting.opt.memory_bank)
